...
namespace AppECommerce.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
        ...
        public ActionResult CheckOut(OrderViewModel req)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    order.Code = "DH" + DateTime.Now.ToString("yyyyMMdd") + rd.Next(0, 9999).ToString("D4");
                    db.Orders.Add(order);
                    ...
                    foreach (var item in cart.Items)
                    {
                        ...
                        if (product != null)
                        {
                            product.Quantity -= item.Quantity; // Reduce stock by the quantity ordered
                        }
                    }
                    db.SaveChanges();
        ...
        //        if (checkProduct.PriceSale > 0 && checkProduct.IsSale)
        //        {
        ...
        public ActionResult AddToCart(int id, int quantity)
        {
            ...
            if (checkProduct != null)
            {
                ...
                ShoppingCartItem item = new ShoppingCartItem
                {
                    ...
                    CategoryName = checkProduct.ProductCategory.Title,
                    Alias = checkProduct.Alias,
                    Price = checkProduct.IsSale && checkProduct.PriceSale > 0 ? (decimal)checkProduct.PriceSale : checkProduct.Price,
                    ProductImg = checkProduct.ProductImage.FirstOrDefault(x => x.IsDefault)?.Image
                };

                // Attempt to add to cart and check success
                bool success = cart.AddToCart(item, quantity, checkProduct.Quantity);
                if (success)
                {
                    Session["Cart"] = cart;
                    response = new
                ...
                else
                {
                    ...
                    {
                        Success = false,
                        msg = $"Chỉ còn {checkProduct.Quantity} sản phẩm có sẵn.",
                        code = 0,
                        Count = cart.Items.Count
                    };
                }
        ...
        public ActionResult Update(int id, int quantity)
        {
            ...
            if (cart != null)
            {
                ...
                if (product != null)
                {
                    ...
                    bool success = cart.UpdateQuantity(id, quantity, product.Quantity);
                    if (success)
                    {
                        //return Json(new { Success = true, msg = "Cập nhật thành công" });
                        response = new { Success = true, msg = "Cập nhật thành công" };
                    }
                    else
                    {
                        //return Json(new { Success = false, msg = $"Chỉ còn {product.Quantity} sản phẩm có sẵn." });
                        response = new { Success = false, msg = $"Chỉ còn {product.Quantity} sản phẩm có sẵn." };
                    }
                }
                else
                {
                    //return Json(new { Success = false, msg = "Sản phẩm không tồn tại." });
                    response = new { Success = false, msg = "Sản phẩm không tồn tại." };
                }
            ...
            else
            {
                ...
                response = new { Success = false, msg = "Giỏ hàng trống." };
            }
            return Json(response);
        }
